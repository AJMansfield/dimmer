...
namespace TriacDimmer {
	...
	namespace detail {
		...
		volatile uint16_t ch_A_up, ch_A_dn, ch_B_up, ch_B_dn;
		...
		{ 0.        ,  0.03225806,  0.06451613,  0.09677419,  0.12903226,
		  ...
		  0.96774194,  1.        };
		const uint8_t lut_length = sizeof(lut_in) / sizeof(float);

		constexpr float interpolate(const float x, const float[] x_table, const float[] y_table, uint8_t table_length);
	}
};